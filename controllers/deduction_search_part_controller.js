...
router.get("/deduction_search_part", authAdmin, function (req, res) {
    connection.query("SELECT * FROM input_truck_payment_db;", function (err, data) {
        if (err) {
            ...
            res.render("deduction_search_part", { b_division: data });
        }
        else {
            res.send('Please login to view this page!');
        }
...
router.get("/deduction_pages/add_part_purchase_deduction/:input_truck_payment_truck_no", function (req, res) {
